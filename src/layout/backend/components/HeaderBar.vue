  ...
  <div class="the-layout-header">
    <div class="the-layout-navbar flex">
      ...
      <div class="user-info-box fvertical">
        <img class="avatar" :src="userInfo.userAvatar || defaultAvatar">
        <span class="the-tag green mgr_10">{{ userInfo.userName || "用户未设置昵称" }}</span>
    ...
    <div class="the-layout-tags" v-if="layoutInfo.showTagsView">
      <Scrollbar>
        <router-link v-for="(item, index) in layoutInfo.historyViews"
          :class="['the-layout-tag', { 'the-layout-tag-on': isActive(item) }]" :key="index + item.path"
